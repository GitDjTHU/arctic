...
def test_mongo_date_range_query():
    ...
    query = TickStore._mongo_date_range_query(
        self, 'sym', DateRange(dt(2014, 1, 2, 0, 0, tzinfo=mktz()),
                               dt(2014, 1, 3, 0, 0, tzinfo=mktz())))

    assert self._collection.aggregate.call_args_list == [call([
     {"$match": {"s": {"$lte": dt(2014, 1, 2, 0, 0, tzinfo=mktz())},
                 "sy": {"$in": ["s1", "s2"]}}},
    ...
    assert query == {'s': {'$gte': dt(2014, 1, 1, 12, 0, tzinfo=mktz()),
...
def test_tickstore_to_bucket_with_image():
    symbol = 'SYM'
    tz = 'UTC'
    initial_image = {'index': dt(2014, 1, 1, 0, 0, tzinfo=mktz(tz)),
                     'A': 123,
                     'B': 54.4,
                     'C': 'DESC'}
    ...
    index = [dt.fromtimestamp(int(i / 1000)).astimezone(tz=mktz(tz)) for i in
    ...
    assert final_image == {'index': data[-1]['index'],
                           'A': 125,
                           'B': 27.2,
                           'C': 'DESC',
                           'D': 0}
...
def test_tickstore_to_bucket_always_forwards_image():
    symbol = 'SYM'
    tz = 'UTC'
    initial_image = {'index': dt(2014, 2, 1, 0, 0, tzinfo=mktz(tz)),
                     'A': 123,
                     'B': 54.4,
                     'C': 'DESC'}
    data = [{'index': dt(2014, 1, 1, 0, 1, tzinfo=mktz(tz)), 'A': 124, 'D': 0}]
...
def get_coldata(coldata):
    ...
    rowmask = np.unpackbits(np.frombuffer(
        decompress(coldata[ROWMASK]), dtype='uint8'))
...
def test_tickstore_pandas_to_bucket_image():
    ...
    assert final_image == {'index': dt(2014, 1, 4, 0, 0, tzinfo=mktz(tz)),
                           'A': 3, 'B': 3.0, 'C': 'DESC', 'D': 1}
