...
from arctic.store._pandas_ndarray_store import (
    PandasDataFrameStore,  PandasPanelStore,  # PandasStore
)
...
def test_panel_converted_to_dataframe_and_stacked_to_write():
    ...
    panel = Mock(shape=(1, 2, 3),
                 axes=[Mock(names=['n%d' % i]) for i in range(3)])
    ...
    with patch.object(PandasDataFrameStore, 'write') as mock_write:
        with patch('arctic.store._pandas_ndarray_store.DataFrame') as DF:
            store.write(sentinel.mlib, sentinel.version, sentinel.symbol,
                        panel, sentinel.prev)
    ...
    mock_write.assert_called_with(
        sentinel.mlib, sentinel.version, sentinel.symbol, DF.return_value,
        sentinel.prev
    )
...
def test_panel_append_not_supported():
    ...
    with raises(ValueError):
        store.append(sentinel.mlib, sentinel.version, sentinel.symbol, panel,
                     sentinel.prev)
...
def test_panel_converted_from_dataframe_for_reading():
    ...
    mock_read.assert_called_with(sentinel.mlib, sentinel.version,
                                 sentinel.symbol)
...
def test_raises_upon_empty_panel_write():
    ...
    with raises(ValueError):
        store.write(sentinel.mlib, sentinel.version, sentinel.symbol, panel,
                    sentinel.prev)


def test_read_multi_index_with_no_ts_info():
    # github #81: old multi-index ts would not have tz info in metadata.
    # Ensure read is not broken
